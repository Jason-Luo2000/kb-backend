...
前置：./deploy.sh 起栈（或本机 uvicorn :8001）+「模型管理」已配 embedding/LLM。运行：python scripts/e2e_demo.py
"""
import os
import sys
import time
...
_SECTIONS = [
    ...
    ("摄取管线", "文档上传后经解析、分块、嵌入、总结、锚点生成五阶段异步管线处理。分块采用 naive 策略按 token 数与重叠率切分，每块沉淀页码与章节路径。嵌入与总结模型可在「模型管理」切换（任意 provider）。"),
]
