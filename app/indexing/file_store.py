...
def purge_file(tenant_id: str, file_id: str) -> tuple[bool, str | None]:
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT tenant_id, storage_key, size_bytes FROM kb_file WHERE id=%s", (file_id,))
            row = cur.fetchone()
            ...
            # 配额回退（同事务）+ 删 PG（CASCADE chunks/summaries/anchors/versions/kb_file_kb）
            from app.quota import meter_delete
            meter_delete(conn, tenant_id, fsize)
